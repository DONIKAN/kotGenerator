...
import xyz.maxom.sample.entities.Token
...
class TokenRequest(private val queue: RequestQueue, private val context: Context){
	private val gson: Gson
	private val collectionType: Type

	init {
		this.gson = Gson()
		this.collectionType = object : TypeToken<DataResponse<Token>>() {}.type
	}

	fun connexion(data: DataRequest<Token>, callback: RequestCallback<Token>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_TOKEN_CONNEXION, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

				val responseRequest = gson.fromJson<DataResponse<Token>(response.toString(), collectionType)
				...
				when {
					...
					else -> callback.onError(context.resources.getString(R.string.request_none_token))
				}

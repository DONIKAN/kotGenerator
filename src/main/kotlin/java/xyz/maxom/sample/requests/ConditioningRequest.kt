...
import xyz.maxom.sample.entities.Conditioning
...
class ConditioningRequest(private val queue: RequestQueue, private val context: Context){
	private val gson: Gson
	private val collectionType: Type

	init {
		this.gson = Gson()
		this.collectionType = object : TypeToken<DataResponse<Conditioning>>() {}.type
	}

	fun getByCriteria(data: DataRequest<Conditioning>, callback: RequestCallback<Conditioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_CONDITIONING_GETBYCRITERIA, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

				val responseRequest = gson.fromJson<DataResponse<Conditioning>(response.toString(), collectionType)
				...
				when {
					...
					else -> callback.onError(context.resources.getString(R.string.request_none_conditioning))
				}
	...
	fun update(data: DataRequest<Conditioning>, callback: RequestCallback<Conditioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_CONDITIONING_UPDATE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)
	...
	fun create(data: DataRequest<Conditioning>, callback: RequestCallback<Conditioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_CONDITIONING_CREATE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)
	...
	fun delete(data: DataRequest<Conditioning>, callback: RequestCallback<Conditioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_CONDITIONING_DELETE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

...
import xyz.maxom.sample.entities.Functionality
...
class FunctionalityRequest(private val queue: RequestQueue, private val context: Context){
	private val gson: Gson
	private val collectionType: Type

	init {
		this.gson = Gson()
		this.collectionType = object : TypeToken<DataResponse<Functionality>>() {}.type
	}

	fun getByCriteria(data: DataRequest<Functionality>, callback: RequestCallback<Functionality>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_FUNCTIONALITY_GETBYCRITERIA, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

				val responseRequest = gson.fromJson<DataResponse<Functionality>(response.toString(), collectionType)
				...
				when {
					...
					else -> callback.onError(context.resources.getString(R.string.request_none_functionality))
				}

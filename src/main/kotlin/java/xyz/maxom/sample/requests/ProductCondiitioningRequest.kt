...
import xyz.maxom.sample.entities.ProductCondiitioning
...
class ProductCondiitioningRequest(private val queue: RequestQueue, private val context: Context){
	private val gson: Gson
	private val collectionType: Type

	init {
		this.gson = Gson()
		this.collectionType = object : TypeToken<DataResponse<ProductCondiitioning>>() {}.type
	}

	fun update(data: DataRequest<ProductCondiitioning>, callback: RequestCallback<ProductCondiitioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_PRODUCTCONDIITIONING_UPDATE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

				val responseRequest = gson.fromJson<DataResponse<ProductCondiitioning>(response.toString(), collectionType)
				...
				when {
					...
					else -> callback.onError(context.resources.getString(R.string.request_none_productcondiitioning))
				}
	...
	fun delete(data: DataRequest<ProductCondiitioning>, callback: RequestCallback<ProductCondiitioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_PRODUCTCONDIITIONING_DELETE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)
	...
	fun generateCode(data: DataRequest<ProductCondiitioning>, callback: RequestCallback<ProductCondiitioning>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_PRODUCTCONDIITIONING_GENERATECODE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

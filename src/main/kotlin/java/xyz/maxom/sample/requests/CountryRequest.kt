...
import xyz.maxom.sample.entities.Country
...
class CountryRequest(private val queue: RequestQueue, private val context: Context){
	private val gson: Gson
	private val collectionType: Type

	init {
		this.gson = Gson()
		this.collectionType = object : TypeToken<DataResponse<Country>>() {}.type
	}

	fun getByCriteria(data: DataRequest<Country>, callback: RequestCallback<Country>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_COUNTRY_GETBYCRITERIA, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

				val responseRequest = gson.fromJson<DataResponse<Country>(response.toString(), collectionType)
				...
				when {
					...
					else -> callback.onError(context.resources.getString(R.string.request_none_country))
				}
	...
	fun activate(data: DataRequest<Country>, callback: RequestCallback<Country>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_COUNTRY_ACTIVATE, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

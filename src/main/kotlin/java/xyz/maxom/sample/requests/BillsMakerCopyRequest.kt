...
import xyz.maxom.sample.entities.BillsMakerCopy
...
class BillsMakerCopyRequest(private val queue: RequestQueue, private val context: Context){
	private val gson: Gson
	private val collectionType: Type

	init {
		this.gson = Gson()
		this.collectionType = object : TypeToken<DataResponse<BillsMakerCopy>>() {}.type
	}

	fun editBills(data: DataRequest<BillsMakerCopy>, callback: RequestCallback<BillsMakerCopy>){
		try {
			val dataObject = JSONObject(gson.toJson(data))

			val request = object : JsonObjectRequest(Method.POST, UrlConstants.URL_BILLSMAKERCOPY_EDITBILLS, dataObject, Response.Listener<JSONObject> { response ->
				Logger.d("REQUEST SUUCESS: ")
				Logger.json(dataObject)

				val responseRequest = gson.fromJson<DataResponse<BillsMakerCopy>(response.toString(), collectionType)
				...
				when {
					...
					else -> callback.onError(context.resources.getString(R.string.request_none_billsmakercopy))
				}
